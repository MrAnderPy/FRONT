...
import fetchData2 from "../components/servicios/fetchData2";
const apiUrl = import.meta.env.VITE_API_URL;
export function DetallePedidoPerfilModal({ id_gestion }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchData2(`${apiUrl}/detalle_pedido_cliente_perfil/${id_gestion}`, token);
        setData(data);
        console.log(data);
        ...
        <DialogBody className="h-[42rem] overflow-scroll">
          <div className="relative flex flex-col ml-4 text-gray-700 bg-white rounded-xl">
            <table className="w-full text-left min-w-full mb-12">
              ...
              <tbody>
                {data.map(({ id_detalle, nombre_producto, precio_unitario, cantidad }) => (
                  <tr key={id_detalle}>
