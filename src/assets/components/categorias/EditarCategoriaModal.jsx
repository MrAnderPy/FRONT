...
export function EditarCategoriaModal({ id_categoria, onUpdateCategory }) {
  ...
  useEffect(() => {
    if (open) {
      fetch(`${apiUrl}/consultar_categoria/${id_categoria}`)
  ...
  const manejarEnvio = async (event) => {
    ...
    try {
      const respuesta = await editar(`${apiUrl}/actualizar_categoria/`, data, id_categoria);
      if (respuesta.estado) {
        Swal.fire('Actualizado!', respuesta.msg, 'success');
        onUpdateCategory(id_categoria, data);
        setOpen(false);
      } else {
