...
export async function generarExcel(data) {
  ...
  const hojaData = data.map((compra) => [
    compra.id_compra,
  ...
  // Encabezados
  sheet.getCell("A6").value = "ID Compra";
  sheet.getCell("B6").value = "Nombre Proveedor";
  sheet.getCell("C6").value = "Fecha Pedido";
  ...
  // Agregar fila para la fecha y hora al final
  ...
  saveAs(blob, "compras.xlsx"); // Cambiar el nombre del archivo si es necesario
}
