...
export function CrearRolModal({ onAddCategory }) {
  ...
  const manejarEnvio = async (event) => {
    ...
    const result = rolSchema.safeParse(datos);
    ...
    if (!result.success) {
      ...
      try {
        const respuesta = await enviarDatos(`${apiUrl}/crear_rol`, datos);
        if (respuesta.estado) {
  ...
  return (
    <>
      <Button color="green" onClick={() => setOpen(true)} variant="gradient" className="flex">
        ...
        <span className="px-2">Nuevo Rol</span>
      </Button>
      <Dialog
        open={open}
